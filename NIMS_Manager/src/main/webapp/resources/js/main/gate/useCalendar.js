$(function() {
		...
		draw: function(){
			...
			let calendar = new FullCalendar.Calendar(calendarContainer, {
					...
					events: function(info, success, fail) {
						AjaxUtil.request({
							...
							data: {
								...
								for(item of data) {
									if(item.pass_in_cnt > 0 && item.pass_in_user_cnt > 0) {
										result.push({
											"title": `${item.pass_in_cnt}회,${item.pass_in_user_cnt}명,pass_in`,
											"start": item.pass_date,
											"displayOrder": 1
										});
									}
									
									if(item.pass_out_cnt > 0 && item.pass_out_user_cnt > 0) {
										result.push({
											"title": `${item.pass_out_cnt}회,${item.pass_out_user_cnt}명,pass_out`,
											"start": item.pass_date,
											"displayOrder": 2
										});
									}
				...
				eventContent: function(args, createElement) {
					...
					let bookTitle = `<span class="calendar-title ta-c"><i class="fas fa-door-open icon" role="${titles[2]}"></i><div class="stat">${titles[0]}</div></span>`;
	...
	Content.load({
		url: '/api/stats/getStatsGateCalendar'
	});
});