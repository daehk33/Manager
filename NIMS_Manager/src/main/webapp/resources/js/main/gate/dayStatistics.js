$(function() {
	const Content = {
		...
		event: function(){
			...
			$('button[role="export"]').click(function(e){
				...
				if(Search.params.type != "all") file_name = "기간별 통계_("+Search.params.startDate+").xlsx";
				else file_name = "기간별 통계_("+Search.params.startDate+"-"+Search.params.endDate+").xlsx";
		...
		draw: function(){
			...
			const table = new Tabulator("#statsTable", {
				...
				columns: [
					...
					{ title: "날짜", field: "pass_date", headerSort: false,
						hozAlign: "center", headerHozAlign: "center", topCalc:() => "합계"
					},
					{
						title: "입실 정보", hozAlign: "center", headerHozAlign: "center",
						columns: [
							{ title: "출입 횟수", field: "pass_in_cnt", headerSort: false, hozAlign: "center", headerHozAlign: "center", topCalc:"sum" },
							{ title: "이용자 수", field: "pass_in_user", headerSort: false, hozAlign: "center", headerHozAlign: "center" },
						]
					},
					{
						title: "퇴실 정보", hozAlign: "center", headerHozAlign: "center",
						columns: [
							{ title: "출입 횟수", field: "pass_out_cnt", headerSort: false, hozAlign: "center", headerHozAlign: "center", topCalc:"sum" },
							{ title: "이용자 수", field: "pass_out_user", headerSort: false, hozAlign: "center", headerHozAlign: "center" },
						]
					}
				],
	...
	const UseStatsChart = {
		...
		draw: function(){
			...
			const table = new Tabulator("#statsChart", {
				...
				ajaxResponse : function(url, params, response) {
					...
					Object.entries(libraryInfo).forEach(info => {
						...
						col.label = info[1].map(library => {
							const dateArr = library.pass_date.split('-');
							if(dateArr.length == 2) return Number(dateArr[1]) + '월';
							else if(dateArr.length == 3) return Number(dateArr[1]) + '월 ' + Number(dateArr[2]) + '일';
							
							return Number(dateArr[dateArr.length - 1]);
						});
						...
						info[1].forEach(library => {
							col.pass_in_info[0].push(library.pass_in_cnt);
							col.pass_in_info[1].push(library.pass_in_user);
							col.pass_out_info[0].push(library.pass_out_cnt);
							col.pass_out_info[1].push(library.pass_out_user);
						});
				...
				columns: [
					{
						title: "입실 정보", field: 'pass_in_info', hozAlign: "center", headerSort: false, headerHozAlign: "center",
						formatter:function(cell, formatterParams, onRendered){
							const data = cell._cell.row.getData();
							const cellElement = `<canvas name="inStats" library="${data.library_key}" height="490px"><canvas>`;
							onRendered(function(){
								let ctx = document.querySelector(`canvas[name="inStats"][library="${data.library_key}"]`).getContext('2d');
								let chartData = {
									datasets: [{
										label: '출입 횟수',
										data: data.pass_in_info[0],
										backgroundColor: '#23cf93',
										borderColor: '#23cf93'
									},{
										label: '이용자 수',
										data: data.pass_in_info[1],
					...
					{
						title: "퇴실 정보", field: 'pass_out_info', hozAlign: "center", headerSort: false, headerHozAlign: "center",
						formatter:function(cell, formatterParams, onRendered){
							const data = cell._cell.row.getData();
							const cellElement = `<canvas name="outStats" library="${data.library_key}" height="490px"><canvas>`;
							onRendered(function(){
								let ctx = document.querySelector(`canvas[name="outStats"][library="${data.library_key}"]`).getContext('2d');
								let chartData = {
									datasets: [{
										label: '출입 횟수',
										data: data.pass_out_info[0],
										backgroundColor: '#23cf93',
										borderColor: '#23cf93'
									},{
										label: '이용자 수',
										data: data.pass_out_info[1],
										backgroundColor: '#7e44ea',
										borderColor: '#7e44ea'
									}],
									labels: data.label,
	...
	Content.load({
		url: '/api/stats/getStatsGateList'
	});
});