$(function() {
	...
	const UseStatsTable = {
		...
		draw : function () {
			...
			const table = new Tabulator("#statsTable", {
				...
				columns: [
					{
						title: "시간", 	field: "pass_time", widthGrow: 2, hozAlign: "center", headerHozAlign: "center", headerSort: false,
					...
					},
					{
						...
						columns: [
							{ title: "출입 횟수", field: "pass_in_cnt", widthGrow: 4, hozAlign:"center", headerHozAlign: "center", headerSort: false },
							{ title: "이용자 수 ", field: "pass_in_user_cnt", widthGrow: 4, hozAlign:"center", headerHozAlign: "center", headerSort: false },
						]
					},
					{
						title: "퇴실 정보", hozAlign: "center", headerHozAlign: "center",
						columns: [
							{ title: "출입 횟수", field: "pass_out_cnt", widthGrow: 4, hozAlign:"center", headerHozAlign: "center", headerSort: false },
							{ title: "이용자 수 ", field: "pass_out_user_cnt", widthGrow: 4, hozAlign:"center", headerHozAlign: "center", headerSort: false },
						]
					},
					{
						title: "종합 정보", hozAlign: "center", headerHozAlign: "center",
						columns: [
							{ title: "누적 출입 횟수", field: "total_cnt", widthGrow: 4, hozAlign:"center", headerHozAlign: "center", headerSort: false, topCalc: "max" },
							{ title: "누적 이용자 수 ", field: "total_user_cnt", widthGrow: 4, hozAlign:"center", headerHozAlign: "center", headerSort: false, topCalc: "max" },
						]
					}
				],
			...
			const table = new Tabulator("#statsChart", {
				...
				ajaxResponse : function(url, params, response) {
					...
					Object.entries(libraryInfo).forEach(info => {
						...
						col.label = info[1].map(library => {
							const label = ('00' + library.log_time).slice(-2) + "시";
						...
						info[1].forEach(library => {
							...
							col.pass_in_info[1].push(library.pass_in_user_cnt);
							col.pass_out_info[0].push(library.pass_out_cnt);
							col.pass_out_info[1].push(library.pass_out_user_cnt);
						});
				...
				columns: [
					...
					},
					{
						...
						formatter: function(cell, formatterParams, onRendered){
							const data = cell._cell.row.getData();
							const cellElement = `<canvas name="returnStats" library="${data.library_key}" height="290px"><canvas>`;
							
							onRendered(function(){
								let ctx = document.querySelector(`canvas[name="returnStats"][library="${data.library_key}"]`).getContext('2d');
								let chartData = {
									datasets: [{
										label: '출입 횟수',
	...
	Content.load({
		url: '/api/stats/getStatsGateTime'
	});
});