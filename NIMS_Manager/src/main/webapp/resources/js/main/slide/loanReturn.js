...
$(function() {
	const Content = {
		...
		load: function(params) {
			...
			this.targetContainer = $('.slide-container[role="loanReturn"]');
		...
		event: function() {
			...
			$('input[name="library"]').change(function() {
				if($('.slide-container[role="loanReturn"]')[0]) {
					reload();
				}
			});
	...
	const BookStatus = {
		...
		load: function(){
			...
			that.loanContainer = $('.card[data-card="loanReturn-total-loan"]');
			that.returnContainer = $('.card[data-card="loanReturn-total-return"]');
			
			AjaxUtil.request({
				url: '/api/dashboard/getLoanReturnStatus',
				done: function(data) {
		...
		draw: function() {
			...
			const returnContainer = that.returnContainer;
			
			const loanCount = loanContainer.find('.card-title[role="count"]');
			const returnCount = returnContainer.find('.card-title[role="count"]');
			
			loanCount.text(data.loan_cnt);
			returnCount.text(data.return_cnt);
		}
	}
	...
	const DangerFeed = {
		...
		event: function(){
			...
			// 확대 버튼 hover 시
			tippy('.slide-container[role="loanReturn"] span[role="show"]', {
				content: "알림 전체보기",
				inertia: true,
			});
	...
	$('#alertModal').on('hide.bs.modal', function(e) {
		if($('.slide-container[role="loanReturn"]')[0]) {
			DangerFeed.load();
			Animation.load($('.slide-container[role="loanReturn"] .feed-box-content'), 'animate__animated animate__flipInX');
		}
	});
			...
			items.forEach(event => {
				container.append(`
					<div class="event-status-container">
						<div class="circle mb-1" id="circles-loanReturn-${event.event_type}"></div>
				...
				let eventStatusCircle = Circles.create({
					id: `circles-loanReturn-${event.event_type}`,
	...
	const TodayStatistics = {
			...
			load: function(){
				...
				AjaxUtil.request({
					url: '/api/dashboard/getLoanReturnUseLogCntbyBook',
					done: function(data){
				...
				AjaxUtil.request({
					url: '/api/dashboard/getLoanReturnUseLogCntbyUser',
					done: function(data){
			...
			draw: function(){
				...
				const todayCtx = document.querySelector('.slide-container[role="loanReturn"] #todayStatisticsChart').getContext('2d');
	...
	const WeekStatistics = {
			...
			load: function(type) {
				...
				// 출입 통계
				AjaxUtil.request({
					url: '/api/dashboard/getLoanReturnUseLogCntbyWeek',
					data: {
			...
			draw: function() {
				...
				let ctx = document.querySelector('.slide-container[role="loanReturn"] #weekStatisticsChart').getContext('2d');
	...
	const Rank = {
		...
		load: function() {
			...
			// 베스트 도서
			AjaxUtil.request({
				url: '/api/loanReturn/getLoanReturnBookRankList',
				data: Object.assign({
					...
					for(item of data) {
						that.bookLabels.push(item.book_title);
			...
			// 우수 회원
			AjaxUtil.request({
				url: '/api/loanReturn/getLoanReturnUserRankList',
				data: Object.assign({
					library_key: sessionStorage.getItem('library_key'),
					limit: 5
				}, params),
		...
		draw: function() {
			...
			const bookCtx = document.querySelector('.slide-container[role="loanReturn"] #bookRankChart').getContext('2d');
			...
			const userCtx = document.querySelector('.slide-container[role="loanReturn"] #userRankChart').getContext('2d');
