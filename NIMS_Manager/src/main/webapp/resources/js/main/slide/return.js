...
$(function() {
	const Content = {
		...
		load: function(params) {
			...
			this.targetContainer = $('.slide-container[role="return"]');
		...
		event: function() {
			...
			$('input[name="library"]').change(function() {
				if($('.slide-container[role="return"]')[0]) {
					reload();
				}
			});
	...
	const ReturnStatus = {
		...
		load: function(){
			...
			that.returnContainer = $('.card[data-card="return-total-return"]');
			that.userContainer = $('.card[data-card="return-total-user"]');
			
			AjaxUtil.request({
				url: '/api/dashboard/getReturnStatus',
				done: function(data) {
					that.data = data.result.items;
					that.draw();
				}
			});
		},
		draw: function() {
			...
			returnCount.text(data.total_return_cnt);
	...
	const DangerFeed = {
		...
		event: function(){
			...
			// 확대 버튼 hover 시
			tippy('.slide-container[role="return"] span[role="show"]', {
				content: "알림 전체보기",
				inertia: true,
			});
	...
	$('#alertModal').on('hide.bs.modal', function(e) {
		if($('.slide-container[role="return"]')[0]) {
			DangerFeed.load();
			Animation.load($('.slide-container[role="return"] .feed-box-content'), 'animate__animated animate__flipInX');
		}
	});
			...
			items.forEach(event => {
				container.append(`
					<div class="event-status-container">
						<div class="circle mb-1" id="circles-return-${event.event_type}"></div>
				...
				let eventStatusCircle = Circles.create({
					id: `circles-return-${event.event_type}`,
	...
	const TodayStatistics = {
		...
		load: function(){
			...
			AjaxUtil.request({
				url: '/api/dashboard/getReturnUseLogCntbyToday',
				done: function(data){
					data = data.result.items;
					
					that.bookData.push(data[0].total_return_cnt);
		...
		draw: function(){
			...
			const todayCtx = document.querySelector('.slide-container[role="return"] #todayStatisticsChart').getContext('2d');
...
//							},
	...
	const WeekStatistics = {
		...
		load: function(type) {
			...
			// 출입 통계
			AjaxUtil.request({
				url: '/api/dashboard/getReturnUseLogCntbyWeek',
				data: {
		...
		draw: function() {
			...
			let ctx = document.querySelector('.slide-container[role="return"] #weekStatisticsChart').getContext('2d');
	...
	const TimeStatistics = {
		...
		load: function(type) {
			...
			// 출입 통계
			AjaxUtil.request({
				url: '/api/dashboard/getReturnUseLogCntbyTime',
				done: function(data){
					let items = data.result.items;
		...
		draw: function() {
			...
			let ctx = document.querySelector('.slide-container[role="return"] #timeStatisticsChart').getContext('2d');
			let statisticsChart = new Chart(ctx, {
				type: 'bar',
				data: {
					labels: that.labels,
