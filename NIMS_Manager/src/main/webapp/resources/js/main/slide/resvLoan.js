...
$(function() {
	const Content = {
		...
		load: function(params) {
			...
			this.targetContainer = $('.slide-container[role="resvLoan"]');
		...
		event: function() {
			...
			$('input[name="library"]').change(function() {
				if($('.slide-container[role="resvLoan"]')[0]) {
					reload();
				}
			});
	...
	const ResvLoanStatus = {
		...
		load: function(){
			...
			that.bookContainer = $('.card[data-card="book-status-book"]');
			that.userContainer = $('.card[data-card="book-status-user"]');
			
			AjaxUtil.request({
				url: '/api/dashboard/getResvLoanStatus',
				done: function(data) {
		...
		draw: function() {
			...
			const bookContainer = that.bookContainer;
			const userContainer = that.userContainer;
			
			const bookCount = bookContainer.find('.card-title[role="count"]');
			const userCount = userContainer.find('.card-title[role="count"]');
			
			bookCount.text(data.total_book_cnt);
	...
	const DangerFeed = {
		...
		event: function(){
			...
			// 확대 버튼 hover 시
			tippy('.slide-container[role="reservationLoan"] span[role="show"]', {
				content: "알림 전체보기",
				inertia: true,
			});
	...
	$('#alertModal').on('hide.bs.modal', function(e) {
		if($('.slide-container[role="resvLoan"]')[0]) {
			DangerFeed.load();
			Animation.load($('.slide-container[role="resvLoan"] .feed-box-content'), 'animate__animated animate__flipInX');
		}
	});
	...
	const EventStatus = {
		...
		load: function(){
			...
			const params = 
				type == 'today' ? {type: 'day', startDate: common.getDateToStr()} : 
			...
			items.forEach(event => {
				container.append(`
					<div class="event-status-container">
						<div class="circle mb-1" id="circles-resvLoan-${event.event_type}"></div>
				...
				let eventStatusCircle = Circles.create({
					id: `circles-resvLoan-${event.event_type}`,
	...
	const TodayStatistics = {
		...
		load: function(){
			...
			AjaxUtil.request({
				url: '/api/dashboard/getResvLoanUseLogCntbyToday',
				done: function(data){
					data = data.result.items;
					
					that.bookData.push(data[0].total_book_cnt);
		...
		draw: function(){
			...
			const todayCtx = document.querySelector('.slide-container[role="resvLoan"] #todayStatisticsChart').getContext('2d');
...
//							},
			...
			Content.charts.todayStatistics = todayChart;
		}
	}
	...
	const WeekStatistics = {
		...
		load: function(type) {
			...
			AjaxUtil.request({
				url: '/api/dashboard/getResvLoanUseLogCntbyWeek',
				data: {
		...
		draw: function() {
			...
			let ctx = document.querySelector('.slide-container[role="resvLoan"] #weekStatisticsChart').getContext('2d');
			...
			Content.charts.weekStatistics = statisticsChart;
		},
	...
	const Rank = {
		...
		load: function() {
			...
			// 베스트 도서
			AjaxUtil.request({
				url: '/api/resvLoan/getResvLoanBookRankList',
				data: Object.assign({
			...
			// 우수 회원
			AjaxUtil.request({
				url: '/api/resvLoan/getResvLoanUserRankList',
				data: Object.assign({
					library_key: sessionStorage.getItem('library_key'),
					limit: 5
				}, params),
		...
		draw: function() {
			...
			const bookCtx = document.querySelector('.slide-container[role="resvLoan"] #bookRankChart').getContext('2d');
			...
			const userCtx = document.querySelector('.slide-container[role="resvLoan"] #userRankChart').getContext('2d');
			...
			Content.charts.bookChart = bookChart;
			Content.charts.userChart= userChart;
		}
	}
