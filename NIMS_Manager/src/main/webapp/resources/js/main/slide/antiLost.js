...
$(function() {
	const Content = {
		...
		load: function(params) {
			...
			this.targetContainer = $('.slide-container[role="antiLost"]');
		...
		event: function() {
			...
			$('input[name="library"]').change(function() {
				if($('.slide-container[role="antiLost"]')[0]) {
					reload();
				}
			});
	...
	const PassStatus = {
		...
		load: function(){
			...
			that.passContainer = $('.card[data-card="antiLost-total-cnt"]');
			that.userContainer = $('.card[data-card="antiLost-total-book"]');
			
			AjaxUtil.request({
				url: '/api/dashboard/getAntiLostStatus',
				done: function(data) {
					that.data = data.result && data.result.info || {};
					that.draw();
				}
			});
		},
		draw: function() {
			...
			passCount.text(data.cnt);
			userCount.text(data.book_cnt);
		}
	}
	...
	const DangerFeed = {
		...
		event: function(){
			...
			// 확대 버튼 hover 시
			tippy('.slide-container[role="antiLost"] span[role="show"]', {
				content: "알림 전체보기",
				inertia: true,
			});
	...
	$('#alertModal').on('hide.bs.modal', function(e) {
		if($('.slide-container[role="antiLost"]')[0]) {
			DangerFeed.load();
			Animation.load($('.slide-container[role="antiLost"] .feed-box-content'), 'animate__animated animate__flipInX');
		}
	});
			...
			items.forEach(event => {
				container.append(`
					<div class="event-status-container">
						<div class="circle mb-1" id="circles-antiLost-${event.event_type}"></div>
				...
				let eventStatusCircle = Circles.create({
					id: `circles-antiLost-${event.event_type}`,
	...
	const TodayStatistics = {
		...
		load: function(){
			...
			AjaxUtil.request({
				url: '/api/dashboard/getAntiLostUseLogCntbyToday',
				done: function(data){
					data = data.result.items || [];
					
					that.cntData.push(data[0].cnt);
					that.bookData.push(data[0].book_cnt);
				}
			});
		...
		draw: function(){
			...
			const todayCtx = document.querySelector('.slide-container[role="antiLost"] #todayStatisticsChart').getContext('2d');
	...
	const WeekStatistics = {
		...
		load: function(type) {
			...
			// 출입 통계
			AjaxUtil.request({
				url: '/api/dashboard/getAntiLostUseLogCntbyWeek',
				data: {
		...
		draw: function() {
			...
			let ctx = document.querySelector('.slide-container[role="antiLost"] #weekStatisticsChart').getContext('2d');
	...
	const TimeStatistics = {
		...
		load: function(type) {
			...
			// 출입 통계
			AjaxUtil.request({
				url: '/api/dashboard/getAntiLostUseLogCntbyTime',
				done: function(data){
					let items = data.result.items;
		...
		draw: function() {
			...
			let ctx = document.querySelector('.slide-container[role="antiLost"] #timeStatisticsChart').getContext('2d');
			let statisticsChart = new Chart(ctx, {
				type: 'bar',
				data: {
					labels: that.labels,
