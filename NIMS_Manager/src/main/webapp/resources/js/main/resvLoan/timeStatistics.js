$(function() {
	...
	const UseStatsTable = {
		...
		draw : function () {
			...
			const table = new Tabulator("#statsTable", {
				...
				columns: [
					{
						title: "시간", 	field: "loan_hour", widthGrow: 2, hozAlign: "center", headerHozAlign: "center", headerSort: false, topCalc:() => "합계",
				...
				ajaxResponse : function(url, params, response) {
					...
					Object.entries(libraryInfo).forEach(info => {
						...
						col.label = info[1].map(library => {
							const label = ('00' + library.loan_hour).slice(-2) + "시";
						...
						info[1].forEach(library => {
							col.loan_info[0].push(library.cnt);
							col.loan_info[1].push(library.total_cnt);
							col.loan_info[2].push(library.user_cnt);
							col.loan_info[3].push(library.total_user_cnt);
						});
							...
							onRendered(function(){
								...
								let chartData = {
									datasets: [{
										...
										label: '이용자 수',
										data: data.loan_info[2],
										backgroundColor: '#efaf34',
										borderColor: '#efaf34'
									},{
										label: '누적 이용자 수',
										data: data.loan_info[3],
	...
	Content.load({
		url: '/api/stats/getStatsResvLoanTime'
	});
});