$(function() {
		...
		draw: function(){
			...
			const table = new Tabulator("#statsTable", {
				...
				columns: [
					...
					{ title: "날짜", field: "loan_date", headerSort: false, hozAlign: "center", headerHozAlign: "center", topCalc:() => "합계" },
					{
				...
				ajaxResponse : function(url, params, response) {
					...
					Object.entries(libraryInfo).forEach(info => {
						...
						col.label = info[1].map(library => {
							const dateArr = library.loan_date.split('-');
	...
	Content.load({
		url: '/api/stats/getStatsResvLoanList'
	});
});