$(function() {
			...
			const table = new Tabulator("#settingTable", {
				...
				ajaxResponse: function(url, params, response) {
					...
					if(response.result.itemsCount == 0) {
						...
						table.updateColumnDefinition("group_1", {headerFilter: false});
						table.updateColumnDefinition("description", {headerFilter: false});
					}
					else {
						$('.card-title-count').show();
						table.updateColumnDefinition("group_1", {headerFilter: true});
						table.updateColumnDefinition("description", {headerFilter: true});
					}
					
					return response.result.items;
				},
				columns: [
					{ title: "NO", field:"manage_id", width: 70, hozAlign: "center", headerHozAlign: "center", headerSort: false , download:false },
					{ title:"정책 그룹명", field:"group_1", widthGrow: 1, headerSort: false, headerFilterPlaceholder: " " },
					{ title: "정책 내용", field:"description", widthGrow: 4, headerSort: false, headerFilterPlaceholder: " " },
					{
						title:"설정 값", field:"value_1", widthGrow: 4, headerSort: false,
	...
	Content.load({
		url: '/api/resvLoan/getResvLoanRuleInfoList'
	});
});