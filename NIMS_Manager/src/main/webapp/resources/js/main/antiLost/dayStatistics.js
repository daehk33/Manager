$(function() {
	const Content = {
		...
		event: function(){
			...
			// 장비 검색 로드
			const deviceParams = {model_key: '5'}
			Device.load(deviceParams);
		...
		draw: function(){
			...
			const table = new Tabulator("#statsTable", {
				...
				columns: [
					...
					{ title: "날짜", field: "date", headerSort: false,
				...
				ajaxResponse : function(url, params, response) {
					...
					Object.entries(libraryInfo).forEach(info => {
						...
						col.label = info[1].map(library => {
							const dateArr = library.date.split('-');
				...
				columns: [
					{
						...
						formatter:function(cell, formatterParams, onRendered){
							...
							const cellElement = `<canvas name="status" library="${data.library_key}" height="490px"><canvas>`;
							onRendered(function(){
								let ctx = document.querySelector(`canvas[name="status"][library="${data.library_key}"]`).getContext('2d');
								let chartData = {
									datasets: [{
										label: '카운트 횟수',
										data: data.info[0],
