...
$(function() {
	...
	const Device = {
		...
		event: function() {
			...
			modal.on('show.bs.modal', function(e) {
				...
				// 각 항목별 입력 타입 설정
				ConfigManager.setFieldType(null, 'rule_value');
		...
		action: function() {
			...
			footer.find('[role="action"]').click(function() {
				...
				const rule_value = $('#rule_value').val();
				
				const inputValues = [rule_value];
				...
				AjaxUtil.request({
					url: '/api/return/updateReturnRuleInfo',
					data: Object.assign({
						rec_key: rec_key,
	...
	Content.load({
		onHide: () => {
			...
			Tabulator.prototype.findTable('#settingTable')[0].setData('/api/return/getReturnRuleInfoList', {}, ajaxConfig).then(function() {
