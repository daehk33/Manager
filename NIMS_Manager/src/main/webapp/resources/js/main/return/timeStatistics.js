$(function() {
	...
	const UseStatsTable = {
		...
		draw : function () {
			...
			const table = new Tabulator("#statsTable", {
				...
				columns: [
					{
						title: "시간", 	field: "return_time", widthGrow: 2, hozAlign: "center", headerHozAlign: "center", headerSort: false, topCalc:() => "합계",
					...
					},
					{
						...
						columns: [
							{ title: "건수", field: "cnt", widthGrow: 4, hozAlign:"center", headerHozAlign: "center", headerSort: false, topCalc: "sum" },
							{ title: "누적 건수", field: "total_cnt", widthGrow: 4, hozAlign:"center", headerHozAlign: "center", headerSort: false, topCalc: "max" },
							{ title: "이용 횟수 ", field: "user_cnt", widthGrow: 4, hozAlign:"center", headerHozAlign: "center", headerSort: false, topCalc: "sum" },
		...
		draw: function() {
			...
			const table = new Tabulator("#statsChart", {
				...
				ajaxResponse : function(url, params, response) {
					...
					const libraryInfo = TableUtil.groupBy(items, 'rec_key');
					...
					Object.entries(libraryInfo).forEach(info => {
						...
						col.label = info[1].map(library => {
							const label = ('00' + library.return_hour).slice(-2) + "시";
						...
						info[1].forEach(library => {
							col.return_info[0].push(library.cnt);
							col.return_info[1].push(library.total_cnt);
							col.return_info[2].push(library.user_cnt);
							col.return_info[3].push(library.total_user_cnt);
						});
				...
				columns: [
					{
						title: "대출 정보", field: 'return_info', hozAlign: "center", headerSort: false, headerHozAlign: "center",
						formatter:function(cell, formatterParams, onRendered){
							const data = cell._cell.row.getData();
							const cellElement = `<canvas name="returnStats" library="${data.library_key}" height="490px"><canvas>`;
							onRendered(function(){
								...
								let chartData = {
									datasets: [{
										...
										label: '이용자 수',
										data: data.return_info[2],
										backgroundColor: '#efaf34',
										borderColor: '#efaf34'
									},{
										label: '누적 이용자 수',
										data: data.return_info[3],
										backgroundColor: '#e85fb6',
										borderColor: '#e85fb6'
									}],
									labels: data.label,
								};
	...
	Content.load({
		url: '/api/stats/getStatsReturnTime'
	});
});