$(function() {
	const Content = {
		...
		event: function(){
			...
			$('button[role="export"]').click(function(e){
				...
				if(Search.params.type != "all") file_name = "예약 대출 이용 통계_("+Search.params.startDate+").xlsx";
				else file_name = "예약 대출 이용 통계_("+Search.params.startDate+"-"+Search.params.endDate+").xlsx";
		...
		draw: function(){
			...
			const table = new Tabulator("#statsTable", {
				...
				columns: [
					...
					{ title: "날짜", field: "return_date", headerSort: false,
					...
					},
					{
						...
						columns: [
							...
							{ title: "이용자 수", field: "user_cnt", headerSort: false, hozAlign: "center", headerHozAlign: "center", topCalc:"sum" }
						]
					}
				],
				...
				ajaxResponse : function(url, params, response) {
					...
					Object.entries(libraryInfo).forEach(info => {
						...
						col.label = info[1].map(library => {
							const dateArr = library.return_date.split('-');
						...
						info[1].forEach(library => {
							...
							col.loan_info[1].push(library.user_cnt);
						});
	...
	Content.load({
		url: '/api/stats/getStatsReservationLoanList'
	});
});