$(function() {
		...
		draw: function(){
			...
			const table = new Tabulator("#statsTable", {
				...
				columns: [
					...
					{ title: "날짜", field: "log_date", headerSort: false,
						hozAlign: "center", headerHozAlign: "center", topCalc:() => "합계"
					},
					{
						title:"대출 정보", hozAlign: "center", headerHozAlign: "center", 
						columns: [
							{ title: "건수", field: "loan_cnt", headerSort: false, hozAlign: "center", headerHozAlign: "center", topCalc:"sum"  },
							{ title: "이용자 수", field: "loan_user", headerSort: false, hozAlign: "center", headerHozAlign: "center" }
						]
					},
					{
						title:"반납 정보" , hozAlign: "center", headerHozAlign: "center",
						columns:[
							{ title: "건수", field: "return_cnt", headerSort: false, hozAlign: "center", headerHozAlign: "center", topCalc:"sum" },
							{ title: "이용자 수", field: "return_user", headerSort: false, hozAlign: "center", headerHozAlign: "center" },
						]
					},
				],
		...
		draw: function() {
			...
			const table = new Tabulator("#statsChart", {
				...
				ajaxResponse : function(url, params, response) {
					...
					Object.entries(libraryInfo).forEach(info => {
						...
						col.label = info[1].map(library => {
							const dateArr = library.log_date.split('-');
						...
						info[1].forEach(library => {
							...
							col.loan_info[1].push(library.loan_user);
							col.return_info[0].push(library.return_cnt);
							col.return_info[1].push(library.return_user);
						});
				...
				columns: [
					...
					{
						title: "반납 정보", field: 'return_info', hozAlign: "center", headerSort: false, headerHozAlign: "center",
						formatter: function(cell, formatterParams, onRendered){
							const data = cell._cell.row.getData();
							...
							onRendered(function(){
								...
								let chartData = {
									datasets: [{
										label: '반납건 수',
										data: data.return_info[0],
										backgroundColor: '#23cf93',
										borderColor: '#23cf93'
									},{
										label: '이용자 수',
										data: data.return_info[1],
										backgroundColor: '#7e44ea',
										borderColor: '#7e44ea'
									}],
									labels: data.label,
	...
	Content.load({
		url: '/api/stats/getStatsLoanReturnList'
	});
});