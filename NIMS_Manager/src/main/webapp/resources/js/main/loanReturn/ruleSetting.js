$(function() {
	const Content = {
		...
		event: function() {
			...
			$('button[role="export"]').click(function(e){
				...
				let file_name = "대출반납기 정책 목록_("+common.getDateToStr()+").xlsx";
		...
		draw: function() {
			...
			const table = new Tabulator("#settingTable", {
				...
				ajaxURLGenerator : function(url, config, params) {
					let device_key = $('#filter-field').val();
					
					url = `${url}?${common.parseParam(params)}&editable=y&grade=${grade}`;
				...
				columns: [
					...
					{
						title:"항목 4", field:"value5", widthGrow: 1.5, headerSort: false,
						formatter: "tickCross", formatter: function(cell, formatterParams, onRendered) {
							let outerText = "";
							cell.getValue() ? outerText = cell.getValue() : outerText = "-";
	...
	Content.load({
		url: '/api/loanReturn/getLoanReturnRuleInfoList'
	});
});