$(function() {
	const Content = {
		...
		result: function(){
			this.table = LocationRankTable.load();
			this.event();
		},
		event: function(){
			...
			$('button[role="export"]').click(function(e){
				...
				if(Search.params.type != "all") file_name = "자료실별 이용 통계_("+Search.params.startDate+").xlsx";
				else file_name = "자료실별 이용 통계_("+Search.params.startDate+"-"+Search.params.endDate+").xlsx";
	...
	const LocationRankTable = {
		...
		draw: function(){
			const table = new Tabulator("#rankTable", {
				...
				placeholder: TableUtil.getPlaceholder('조회된 자료실별 통계가 없습니다.'),
				pagination : "local",
				...
				columns: [
					...
					{ title: "자료실명", field: "device_location", widthGrow: 3, headerSort: false, headerFilterPlaceholder: " " },
					...
					{ title: "대출 횟수", field: "loan_cnt", widthGrow:3 , headerSort: false,
						hozAlign: "center", headerHozAlign: "center",
					},
					{ title: "반납 횟수", field: "return_cnt", widthGrow:3 , headerSort: false,
						hozAlign: "center", headerHozAlign: "center",
					}
				],
	...
	Content.load({
		url: '/api/loanReturn/getLoanReturnLocationRankList'
	});
});