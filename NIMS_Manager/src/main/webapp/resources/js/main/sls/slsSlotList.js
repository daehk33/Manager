	...
	const Content = {
		...
		event: function(){
			...
			$('button[role="export"]').click(function(e) {
				...
				let file_name = "선반 조회_("+common.getDateToStr()+").xlsx";
	...
	const Table = {
		...
		draw: function () {
			...
			const table = new Tabulator("#listTable", {
				...
				history: true,
				movableColumns: true,
				placeholder: TableUtil.getPlaceholder('조회된 항목이 없습니다.'),
				...
				columns: [
					...
					{ title: "서가 위치", field: "drumtray", widthGrow: 1, headerSort: false, hozAlign: "center", headerHozAlign: "center", frozen: true },
					{ title: "선반 번호", field: "slot", widthGrow: 1, hozAlign: "center", headerHozAlign: "center", headerSort: false, frozen: true },
					{
						title: "도서명", field: "title", widthGrow: 4, headerSort: false, headerFilterPlaceholder: " ",
						formatter: "tickCross", formatter: function(cell, formatterParams, onRendered) {
							return cell.getValue() || "-";
						}
					},
					{
						title: "저자", field: "author", widthGrow: 3, headerSort: false, headerFilterPlaceholder: " ",
						formatter: "tickCross", formatter: function(cell, formatterParams, onRendered) {
							return cell.getValue() || "-";
						}
					},
