$(function() {
			...
			const table = new Tabulator("#listTable", {
				...
				ajaxResponse: function(url, params, response) {
					...
					if(response.result.count == 0) {
						...
						table.updateColumnDefinition("book_title", {headerFilter: false});
						table.updateColumnDefinition("member_no", {headerFilter: false});
						table.updateColumnDefinition("member_nm", {headerFilter: false});
						table.updateColumnDefinition("isoverdue", {headerFilter: false});
					}
					else { 
						$('.card-title-count').show();
						table.updateColumnDefinition("book_title", {headerFilter: true});
						table.updateColumnDefinition("member_no", {headerFilter: true});
						table.updateColumnDefinition("member_nm", {headerFilter: true});
						table.updateColumnDefinition("isoverdue", {headerFilter: "select"});
					}
				...
				columns: [
					...
					{ title: "도서명", widthGrow: 4, field: "book_title", headerSort: false, headerFilterPlaceholder: " " },
					{ title: "대출 일시", widthGrow: 2.5, field: "loan_date", headerSort: false },
					{ title: "반납 일시", widthGrow: 2.5, field: "return_date", headerSort: false },
					{ title: "회원 번호", widthGrow: 1.5, field: "member_no", headerSort: false, headerFilterPlaceholder: " " },
					{
						title: "회원명", widthGrow: 1.5, field: "member_nm", headerSort: false, headerFilterPlaceholder: " ",
						formatter: function(cell) {
							return Masking.name(cell.getValue());
						}
					},
					{
						title: "연체 상태", widthGrow: 2, field: "isoverdue", headerSort: false, headerFilterParams: {"0": "정상", "1": "연체"}, headerFilterPlaceholder: " ",
						formatter: function(cell) {
							return cell.getValue() == '0'? "정상" : "연체";
						}
					},
				],
	...
	Content.load({
		url: '/api/history/getLoanHistoryList'
	});
});